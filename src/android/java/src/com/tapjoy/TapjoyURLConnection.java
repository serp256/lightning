...
public class TapjoyURLConnection
{
	...
	{
		return getResponseFromURL(url, params, TYPE_GET);
	}
	...
	public TapjoyHttpURLResponse getResponseFromURL(String url, String params, int type)
	{
		...
		try
		{
			...
			if (contentLength != null)
			{
				try
				{
					tapjoyResponse.contentLength = Integer.valueOf(contentLength);
				}
				catch (Exception e)
				{
		...
		catch (Exception e)
		{
			...
			// Try to get the error code and error content.
			try
			{
				// Check connection is valid.
				if (connection != null && tapjoyResponse.response == null)
				{
					// Read the error result from the server.
					rd = new BufferedReader(new InputStreamReader(((HttpURLConnection) connection).getErrorStream()));
					sb = new StringBuilder();
