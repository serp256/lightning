...
public class TJCOffersWebView extends Activity
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		if (extras != null)
		{
			...
			else
			{
				...
				{
					// Is this device on wifi?  Or are cached videos disabled?  Then show all videos.
					if (TapjoyConnectCore.getConnectionType().equals("wifi") || TapjoyConnectCore.isVideoCacheEnabled() == false)
					{
						// all_videos=true
	...
	protected void onResume()
	{
		...
		// Call connect on resume.
		if (resumeCalled && TapjoyConnectCore.getInstance() != null)
		{
			TapjoyLog.i(TAPJOY_OFFERS, "call connect");
	...
	private class TapjoyWebViewClient extends WebViewClient
	{
		...
		public boolean shouldOverrideUrlLoading(WebView view, String url)
		{
			...
			if (url.startsWith("tjvideo://"))
			{
				...
				webviewURL = params.get("video_complete_url");
				...
				TapjoyLog.i(TAPJOY_OFFERS, "webviewURL: " + webviewURL);
				
				if (TapjoyVideo.getInstance().startVideo(videoID, currencyName, currencyAmount, clickURL, webviewURL))
				{
