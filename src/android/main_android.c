    ...
    switch (cmd) {
        ...
        case APP_CMD_PAUSE:
            ...
            foreground_called_on_resume = 0;
						//on some devices (only on some or its truth for all?) on pause called before focus lost and surface destroyed. without setting animating to false, next
						//frame drawing begun. at the same time in main app thread called focus lost and surface destroyed. frame drawing includes net_run call, in farm it means
						//call protocol packets handling. protocol packets handling can include display objects manipulations, even with underlying gl calls, which failed, because surface destroyed 
						engine->animating = 0; 
            if (engine->stage) {
                sound_android_onbackground();
                if (!bg_handler) {
									bg_handler = caml_hash_variant("dispatchBackgroundEv");
								}
								PRINT_DEBUG("-->>call backgrond callback");
...
void android_main(struct android_app* state) {
    ...
    while (1) {
        ...
        //this is for galaxy note II: without sleep code from mail java thread never called
        usleep(150);
    }
