...
(** Run the event manager in a separate thread. Link with the Ocaml thread library if using this *)
(** Initialize all of the above subsystems, except for EVENTTHREAD *)
...
value init : list init_flag -> unit;
...
module Video :
  sig
    ...
    value lock_surface : surface -> unit;
    (** Unlock surface*)
    value unlock_surface : surface -> unit;
    ...
    value video_mode_ok : int -> int -> int -> list video_flag -> bool;
    ...
    value set_video_mode : int -> int -> int -> list video_flag -> surface;
    ...
    value set_color_key : surface -> list video_flag -> int32 -> unit;
    ...
    value set_alpha : surface -> list video_flag -> int -> unit;
    ...
    value display_format : surface -> surface;
    ...
    value get_rgb : surface -> int32 -> (int * int * int);
    ...
    value get_rgba : surface -> int32 -> (int * int * int * int);
    ...
    value map_rgb : surface -> int -> int -> int -> int32;
    (** [map_rgba surface -> red -> green -> blue -> alpha -> pixel]
		...
		If the format has a palette (8-bit) the index of the closest matching color in the palette will be returned.
		If the specified pixel format has an alpha component it will be returned as all 1 bits (fully opaque). *)
    value map_rgba : surface -> int -> int -> int -> int -> int32;
    (** Rectangle type, used for clipping and blitting operations *)
    type rect =
      { rect_x : mutable int; (** top left *) rect_y : mutable int;
        (** top *) rect_w : mutable int; (** width *) rect_h : mutable int
      };
    (** height *)
    (** [fill_surface surface  pixel]
		Fills the surface with pixels of the given color *)
    value fill_surface : surface -> int32 -> unit;
    ...
    value fill_rect : surface -> rect -> int32 -> unit;
    (** [update_surface surface]
		Makes sure the screen is updated *)
    value update_surface : surface -> unit;
    ...
    value update_rect : surface -> int -> int -> int -> int -> unit;
    ...
    value update_rects : surface -> array rect -> unit;
    (** [flip surface]
    ...
    (** Color type *)
    type color =
      { red : int; (** 0..255 *) green : int; (** 0..255 *) blue : int
      };
    (** 0..255 *)
    ...
    value set_colors : surface -> array color -> int -> int -> bool;
    (** [show_cursor true\false]
		Toggle whether or not the cursor is shown on the screen. *)
    value show_cursor : bool -> unit;
    (**	[warp_mouse x y]
		Set the position of the mouse cursor (generates a mouse motion event). *)
    value warp_mouse : int -> int -> unit;
    (**	[string_of_pixels surface -> string]
		Returns a copy of the raw pixel data in a surface as a string. *)
    value string_of_pixels : surface -> string;
  end;
(*----------------------------- Window management --------------------------------- *)
...
module Window :
  sig
    ...
    value iconify_window : unit -> unit;
    ...
    value toggle_fullscreen : Video.surface -> unit;
...
module SDLGL :
  sig
    ...
    value set_attribute : gl_attr -> int -> unit;
    ...
    value get_attribute : gl_attr -> int;
  end;
(*---------------------------------------------------- Events ---------------------------------------------- *)
...
module Event :
  sig
    ...
    value get_app_state : unit -> list app_state;
    (**	SDLKey enum 
    ...
    value enable_unicode : que_dis_ena -> off_on;
    (**	Default keyboard repeat delay *)
    value default_repeat_delay : int;
    (**	Default keyboard repeat interval *)
    value default_repeat_interval : int;
    ...
    type keyboard_event =
      { keystate : press_release;
        (** The key state ([PRESSED] or [RELEASED]) *) scancode : int;
        (** The key scan code *) sym : key; (** The key sym *)
        modifiers : list key_mod; (** Key modifiers (CTRL, ALT etc)*)
        unicode : int
      };
    ...
    type mouse_motion_event =
      { mousestate : press_release;
        (** The mouse button state ([PRESSED] or [RELEASED]) *) mx : int;
        (** x coordinate of cursor *) my : int; (** y coordinate of cursor *)
        mxrel : int;
    ...
    type mouse_button_event =
      { mousebutton : mouse_button;
        ...
        buttonstate : press_release; (** Pressed\Released*) bx : int;
        (** x coordinate of cursor *) by : int
      };
    (** y coordinate of cursor *)
    (** Joystick axis event *)
    type joy_axis_event =
      { which_axis : int; (** Which joystick generated the event *)
        axis : int; (** Which joystick axis, X (0) or Y (1) *) jvalue : int
      };
    (** The current position along that axis *)
    (** Joystick ball event *)
    type joy_ball_event =
      { which_ball : int; (** Which joystick generated the event *)
        ball : int; (** Which joystick ball *) jxrel : int;
    ...
    (** Joystick hat event *)
    type joy_hat_event =
      { which_hat : int; (** Which joystick generated the event *) hat : int;
        (** Which joystick hat *) hvalue : int
      };
    (** The current hat position *)
    (** Joystick button event *)
    type joy_button_event =
      { which_button : int; (** Which joystick generated the event *)
        joybutton : int; (** Which joystick button *) jstate : press_release
      };
    (** Pressed\Released *)
    (** Window resize event *)
    type resize_event = { w : int; (** New width *) h : int };
    (** New height *)
...
module Audio :
  sig
    ...
    (** 16 bits unsigned *)
    (** 16 bits signed *)
    (** 16 bits unsigned least-significant-bit first*)
    (** 16 bits signed least-significant-bit first*)
    (** 16 bits unsigned most-significant-bit first *)
    (** 16 bits signed most-significant-bit first *)
    ...
    type audio_spec =
      { frequency : int;
        ...
        silence : int; samples : int;
        (** When used with [open_audio] this refers to the size of the audio buffer in samples. A sample a chunk of audio data of the size specified in format mulitplied by the number of channels. When the [audio_spec] is used with [load-wav] samples is set to 4096.  *)
        size : int
      };
    (** The maximum sample mixer volume. Volume ranges from 0 (silence) to mix_maxvolume *)
    value mix_maxvolume : int;
    ...
    value open_audio : audio_spec -> (byte_array -> unit) -> audio_spec;
    (** [close_audio] closes the audio device *)
    value close_audio : unit -> unit;
    ...
    value load_wav : string -> (audio_spec * byte_array);
    (** [free_wav wav_buffer] 
		Frees a WAV buffer allocated by load-wav *)
    value free_wav : byte_array -> unit;
    (** [pause_audio on\off] 
		Toggle audio playback on and off *)
    value pause_audio : bool -> unit;
    ...
    value lock_audio : unit -> unit;
    (**	Unlocks a previous [lock_audio] call.*)
    value unlock_audio : unit -> unit;
    ...
    value convert_audio :
      sample_type ->
        channel_type ->
          int ->
            sample_type -> channel_type -> int -> byte_array -> byte_array;
    ...
    value fx_pan : float -> float -> byte_array -> byte_array;
    ...
    value fx_shift : float -> byte_array -> byte_array;
  end;
(* ---------------------------------------------------- Drawing.-------------------------------------------------- *)
...
module Draw :
  sig
    ...
    type sfont =
      { font_list : list (int * Video.rect);
        (** associative list of ASCII char values and corresponding SDL rects*)
        font_surf : Video.surface;
        (** Texture map containing font characters *) font_space : int;
        (** Size of the space character ' ' in pixels. Default is the same size as 'L' *)
        font_letters : int;
        (** Space between letters in pixels. Default is the same size as '!' *)
        font_line : int
      };
    (** Space between lines in pixels. Default is zero i.e. font design handles it *)
    ...
    type filter =
      ...
      | LANCZOS3 of int ];
    value box : filter;
    value triangle : filter;
    value bell : filter;
    value bspline : filter;
    value hermite : filter;
    value mitchell : filter;
    value lanczos3 : filter;
    (** [put_pixel surface x y pixel]
		puts an int32 rgb(a) pixel, such as output of [map_rgb(a)], on surface [surface] at location [(x,y)] *)
    value put_pixel : Video.surface -> int -> int -> int32 -> unit;
    (**	[get_pixel surface x y -> pixel]
		gets an int32 rgb(a) pixel, from surface [surface] at location [(x,y)] *)
    value get_pixel : Video.surface -> int -> int -> int32;
    ...
    value scale : Video.surface -> float -> filter -> Video.surface;
    ...
    value scale_to : Video.surface -> int -> int -> filter -> Video.surface;
    ...
    value read_tga : string -> (int * int * int * string * tga_orientation);
    (**	[load_tga file -> surface]
		Loads a TGA file and returns a surface with the image data *)
    value load_tga : string -> Video.surface;
    ...
    value make_mipmaps : Video.surface -> filter -> array Video.surface;
  end;
