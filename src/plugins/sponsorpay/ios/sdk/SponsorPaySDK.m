...
{
    return [[self instance] interstitialViewController];
}

+ (SPInterstitialViewController *)interstitialViewControllerForCredentials:(NSString *)credentialsToken
{
    return [[self instance] interstitialViewControllerForCredentials:credentialsToken];
}
...
{
    return [[self instance] startLoadingInterstitialWithParentViewController:parent];
}
...
- (void)showPayoffNotificationForAmount:(double)amount currencyName:(NSString *)explicitCurrencyName
{
    ...
    [[[[iToast makeText:[NSString stringWithFormat:SPCoinsNotificationText, flooredAmount, currencyName]]
...
- (SPInterstitialViewController *)interstitialViewControllerForCredentials:(NSString *)credentialsToken
{
    ...
    interstitialVC.currencyName = credentials.userConfig[SPCurrencyNameConfigKey];

    [self.elementsWaitingForDisposal addObject:interstitialVC];
    interstitialVC.disposalBlock = ^(void) {
        [[SPLogger defaultLogger] log:@"disposing of Interstitial VC"];
        [self.elementsWaitingForDisposal removeObject:interstitialVC];
    };
