...
NSString *const SPSBJsonParsingErrorDomain = @"SPSBJsonParsingErrorDomain";
#endif
...
- (id)parseJSONResponseWithError:(NSError **)error
{
    ...
    responseAsJson = [parser objectWithData:self.responseData];
    
    if (!responseAsJson && *error) {
        *error = [NSError errorWithDomain:SPSBJsonParsingErrorDomain
                                     code:-1
