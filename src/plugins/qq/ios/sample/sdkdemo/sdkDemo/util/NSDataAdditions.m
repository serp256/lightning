...
- (NSString *)base16Encoding
{
    ...
    char *cstrHex = malloc(len * 2);
    if (NULL == cstrHex)
    {
        return nil;
    }
    
    char *pcBuf = (char *)[self bytes];
    int nData = 0;
    char *pcHex = cstrHex;
    for (int i = 0; i < len; ++i)
    {
    ...
    return [[NSString alloc] initWithBytesNoCopy:cstrHex length:(pcHex - cstrHex) encoding:NSASCIIStringEncoding freeWhenDone:YES];
}
