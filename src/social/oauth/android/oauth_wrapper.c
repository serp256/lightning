...
void ml_authorization_grant(value url, value closeBt) {
	...
	jclass oauthCls = engine_find_class("ru/redspell/lightning/OAuth");
...
JNIEXPORT void JNICALL Java_ru_redspell_lightning_OAuthDialog_onRedirect(JNIEnv *env, jobject this, jstring url) {
	RUN_ON_ML_THREAD(&oauth_redirect, (void*)(*env)->NewGlobalRef(env, url));
}

JNIEXPORT void JNICALL Java_ru_redspell_lightning_OAuthDialog_onClose(JNIEnv *env, jobject this, jstring url) {
	RUN_ON_ML_THREAD(&oauth_redirect, (void*)(*env)->NewGlobalRef(env, url));
}