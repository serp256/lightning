...
module Make (P:sig value auth_endpoint: string; value token_endpoint: string; value close_button: option close_button; end) = struct
  ...
  *)
  ...
  value get_token_by_code code client_id redirect_uri params callback = 
  ...
  value authorization_grant gtype client_id redirect_uri params callback = 
    match !state with
    [ Standby ->
