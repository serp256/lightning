module type S = sig
  ...
  value img: ?width:float -> ?height:float -> ?paddingLeft:float -> ?paddingTop:float -> ?paddingRight:float -> ?paddingLeft:float -> ?valign:img_valign -> #D.c -> simple_element;
  ...
  value parse_simples: ?imgLoader:(string -> D.c) -> string -> simple_elements;
  value parse: ?imgLoader:(string -> D.c) -> string -> main;
  value create: ?width:float -> ?height:float -> ?border:int -> ?dest:#Sprite.c -> main -> ( (float*float) * D.c);
