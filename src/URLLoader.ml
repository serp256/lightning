...
value download_native ?compress ~url ~path ?ecallback ?progress callback = ();
ELSE
...
IFDEF ANDROID THEN
external download_native: bool -> string -> string -> option (int -> string -> unit) -> option (~progress:float -> ~total:float -> unit -> unit) -> (list string -> unit) -> unit = "ml_DownloadNative_byte" "ml_DownloadNative";
